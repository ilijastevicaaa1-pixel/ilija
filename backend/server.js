...
// 3. DPV výpočet - vypočítava DPH z částky
app.post('/api/vat/calculate', async (req, res) => {
   const { amount, vat_rate, type } = req.body;
   ...
   if (!vat_rate || isNaN(vat_rate) || parseFloat(vat_rate) < 0 || parseFloat(vat_rate) > 100) {
       return res.status(400).json({ error: 'Neplatná sadzba DPH. Použite hodnotu medzi 0 a 100.' });
   }
   ...
   if (type === 'net_to_gross') {
       // Z čisté na hrubou: DPH = amount * vat_rate, spolu = amount + DPH
       const vatAmount = amountNum * vatMultiplier;
       const grossAmount = amountNum + vatAmount;
       result = {
           net_amount: amountNum,
           vat_rate: vatRateNum,
           vat_amount: vatAmount,
           gross_amount: grossAmount,
           description: 'Výpočet DPH z čistej částky na hrubú'
       };
   } else if (type === 'gross_to_net') {
       // Z hrubé na čistou: DPH = amount * (vat_rate / (100 + vat_rate)), spolu = amount - DPH
       const vatAmount = amountNum * (vatMultiplier / (1 + vatMultiplier));
       ...
       result = {
           ...
           description: 'Výpočet DPH z hrubej částky na čistú'
       };
   } else if (type === 'vat_only') {
       // Len výpočet DPH z čisté částky
       const vatAmount = amountNum * vatMultiplier;
       const grossAmount = amountNum + vatAmount;
       result = {
           net_amount: amountNum,
           ...
           vat_amount: vatAmount,
           gross_amount: grossAmount,
           description: 'Výpočet len DPH údaje'
       };
   } else {
       // Default: predpokladáme, že amount je čistená časť
       const vatAmount = amountNum * vatMultiplier;
       const grossAmount = amountNum + vatAmount;
       result = {
           net_amount: amountNum,
           ...
           vat_amount: vatAmount,
           gross_amount: grossAmount,
           description: 'Výpočet DPH (predpokladaná čistená časť)'
       };
   }
   ...
   try {
       ...
       if (db) {
           await db.query(
               'INSERT INTO logs (type, message, timestamp) VALUES ($1, $2, $3)',
               ['info', `DPH výpočet: ${type || 'default'} ${amountNum}€ sadzba ${vatRateNum}%`, new Date()]
           );
       } else {
           console.log(`[LOG] DPH výpočet: ${type || 'default'} ${amountNum}€ sadzba ${vatRateNum}%`);
       }
   } catch (e) {
       console.log('Log error', e);
   }
