...
function ContractsScreen() {
  ...
  function resetFilters() {
    setFilter({ text: '', dateFrom: '', dateTo: '', amountFrom: '', amountTo: '', partner: '' });
    setPage(1);
  }

  let filtered = contracts.filter(c => {
    if (filter.text && !(
      (c.contract_name || '').toLowerCase().includes(filter.text.toLowerCase()) ||
      (c.partner || '').toLowerCase().includes(filter.text.toLowerCase()) ||
      (c.description || '').toLowerCase().includes(filter.text.toLowerCase())
    )) return false;
    if (filter.partner && !(c.partner || '').toLowerCase().includes(filter.partner.toLowerCase())) return false;
    if (filter.dateFrom && c.date_start < filter.dateFrom) return false;
    if (filter.dateTo && c.date_start > filter.dateTo) return false;
    if (filter.amountFrom && Number(c.amount) < Number(filter.amountFrom)) return false;
    if (filter.amountTo && Number(c.amount) > Number(filter.amountTo)) return false;
    return true;
  });
  ...
  async function exportCSV() {
    setExporting(true);
    const data = filtered;
    const header = ['Naziv','Partner','Iznos','Početak','Opis'];
    const rows = data.map(c => [c.contract_name, c.partner, c.amount, c.date_start, c.description]);
  ...
  async function exportXLSX() {
    ...
    XLSX.writeFile(wb, 'ugovori.xlsx');
    setExporting(false);
  }
  ...
  const columns = [
    { field: 'contract_name', label: 'Naziv' },
    { field: 'partner', label: 'Partner' },
    { field: 'amount', label: 'Iznos' },
    { field: 'date_start', label: 'Početak' },
    ...
    <div style={{maxWidth: 900, margin: '40px auto', display: 'flex', gap: 32}}>
      <div style={{flex: 1}}>
        ...
        <div style={{marginBottom: 16, display: 'flex', gap: 8, flexWrap: 'wrap'}}>
          ...
          <InstantSearchInput name="partner" value={filter.partner} onChange={v => handleFilterChange({ target: { name: 'partner', value: v } })} placeholder="Partner" style={{ width: 100 }} />
          ...
          <select value={groupBy} onChange={e=>{setGroupBy(e.target.value);setGroupBy2('');}} style={{marginLeft:8}}>
            ...
            <option value="partner">Po partneru</option>
            <option value="date_start">Po datumu</option>
          </select>
          {groupBy && (
            <select value={groupBy2} onChange={e=>setGroupBy2(e.target.value)} style={{marginLeft:8}}>
              <option value="">Bez podgrupisanja</option>
              {['partner','date_start'].filter(g=>g!==groupBy).map(g=>(<option key={g} value={g}>{g==='partner'?'Po partneru':'Po datumu'}</option>))}
            </select>
          )}
        ...
        {loading ? (
          ...
          grouped.map(g1 => (
            <div key={g1.key} style={{marginBottom:16}}>
              <div style={{fontWeight:'bold',margin:'8px 0'}}>{groupBy==='partner'?'Partner':'Datum'}: {g1.key}</div>
              {groupBy2 && g1.items && Array.isArray(g1.items) && g1.items.length && typeof g1.items[0] === 'object' && g1.items[0][groupBy2] !== undefined ? (
                groupData(g1.items, [groupBy2]).map(g2 => (
                  <div key={g2.key} style={{marginLeft:16,marginBottom:8}}>
                    <div style={{fontWeight:'bold',margin:'4px 0'}}>{groupBy2==='partner'?'Partner':'Datum'}: {g2.key}</div>
                    <VirtualizedTable
                      columns={columns}
                      data={g2.items}
