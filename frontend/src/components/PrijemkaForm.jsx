...
export default function PrijemkaForm({ items, onSubmit, onCancel }) {
  ...
  function handleSubmit(e) {
    ...
    if (price === "" || Number(price) < 0) return setError("Cena mora biti ≥ 0");
    setError("");
    onSubmit({ item_id: itemId, quantity, price, date });
  }
  ...
  return (
    <form onSubmit={handleSubmit} style={{ minWidth: 320 }}>
      ...
      <div>
        <label>Cena</label>
        <input type="number" value={price} onChange={e => setPrice(e.target.value)} required min={0} step={0.01} style={{ width: "100%" }} />
      </div>
      <div>
        <label>Datum</label>
