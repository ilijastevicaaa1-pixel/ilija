...
function InputInvoiceScreen() {
  ...
  const handleFallbackAnalyze = async () => {
    ...
    setError('');
    try {
      const res = await apiFetch('/api/ledger/suggest', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          description: form.description || '',
          amount: form.amount_without_vat,
      ...
      setError('Greška u fallback analizi.');
    }
  ...
  const handleAiAnalyze = async () => {
    ...
    try {
      const res = await apiFetch('/api/ai/command', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: `Analiziraj fakturu: dobavljač ${form.supplier}, iznos ${form.amount_without_vat}, PDV ${form.vat_amount}` })
      });
      const data = await res.json();
  ...
  function validateForm(f) {
    ...
    if (!f.amount_without_vat || isNaN(f.amount_without_vat) || Number(f.amount_without_vat) <= 0) return 'Iznos bez PDV mora biti pozitivan broj';
    if (!f.vat_amount || isNaN(f.vat_amount) || Number(f.vat_amount) < 0) return 'Iznos PDV mora biti broj';
    if (!f.total_amount || isNaN(f.total_amount) || Number(f.total_amount) <= 0) return 'Ukupno mora biti pozitivan broj';
