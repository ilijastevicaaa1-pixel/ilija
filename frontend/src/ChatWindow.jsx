...
function setSceneForCategory(cat, setScene) {
  const map = {
    fakturacia: 'desk',
    banka: 'bank',
    dph: 'office',
...
function ChatWindow({ onClose }) {
    // State za specijalni podmeni "prehľad všetkých faktúr"
    const [awaitingFakturySub, setAwaitingFakturySub] = useState(false);
  ...
  const sendMessage = async () => {
    ...
    if (!selectedCategory) {
      ...
      if (cat) {
        ...
        setSceneForCategory(cat, setScene);
    ...
    // Ako čekamo podopcije za prehľad všetkých faktúr
    if (awaitingFakturySub) {
      const subNum = userText.trim();
      let prompt = "";
      if (subNum === "1") prompt = "Používateľ si vybral možnosť: Zoznam vydaných faktúr v kategórii fakturácia.\nOdpovedaj výhradne po slovensky.";
      else if (subNum === "2") prompt = "Používateľ si vybral možnosť: Zoznam prijatých faktúr v kategórii fakturácia.\nOdpovedaj výhradne po slovensky.";
      else if (subNum === "3") prompt = "Používateľ si vybral možnosť: Stav platby faktúr v kategórii fakturácia.\nOdpovedaj výhradne po slovensky.";
      else if (subNum === "4") prompt = "Používateľ si vybral možnosť: Filtrovanie podľa dátumu, čísla faktúry, či stavu platby v kategórii fakturácia.\nOdpovedaj výhradne po slovensky.";
      else {
        const reply = "Neplatná možnosť. Zadajte 1, 2, 3 alebo 4.";
        setMessages(prev => [...prev, { sender: 'bot', text: reply }]);
        speak(reply);
        setAwaitingFakturySub(false);
        setInput('');
        setLoading(false);
        return;
      }
    ...
    if (selectedCategory) {
      ...
      if (options && options[num]) {
        ...
        // Specijalni prikaz za "prehľad všetkých faktúr" u fakturacia
        if (selectedCategory === "fakturacia" && num === "5") {
          const fakturyText =
            "Prehľad všetkých faktúr v kategórii fakturácia:\n" +
            "- 1 Zoznam vydaných faktúr\n" +
            "- 2 Zoznam prijatých faktúr\n" +
            "- 3 Stav platby faktúr\n" +
            "- 4 Možnosť filtrovania podľa dátumu, čísla faktúry, či stavu platby\n" +
            "Chcete zobraziť konkrétny zoznam alebo filtrovať podľa určitých kritérií?";
          setMessages(prev => [...prev, { sender: 'bot', text: fakturyText }]);
          speak(fakturyText);
          setAwaitingFakturySub(true);
          setInput('');
          setLoading(false);
          return;
        }
  ...
  return (
    <div className="chat-panel">
      <div className="chat-header">
        ...
        <span className="chat-title">Asistent</span>
      ...
      {/* Assistant avatar with scene (optional, can be added above messages if desired) */}
